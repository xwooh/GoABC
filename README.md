Record something when learning golang ~

## 参考文档
[Tony Bai · Go 语言第一课](https://time.geekbang.org/column/intro/100093501)
[极客兔兔](https://github.com/geektutu/high-performance-go)
[架构小白](https://blog.frognew.com/)

## 目录

### 基本概念

1. [项目结构](./01_基本概念/01-01_项目结构.md)
2. [理解模块](./01_基本概念/01-02_模块.md)
3. [程序执行](./01_基本概念/01-03_程序执行.md)

### 基本数据类型

1. [数组和切片](./02_基本数据类型/02-01_数组和切片.md)
2. [map](./02_基本数据类型/02-02_map.md)
3. [结构体](./02_基本数据类型/02-03_结构体.md)
4. [chan](./02_基本数据类型/02-04_chan.md)

### 并发模型
1. [调度器模型与演化过程](./03_并发模型/03-01_调度器模型与演化过程.md)
